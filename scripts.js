...
(function () {
  ...
  /* --- events page --------------------------------------------
     The cards are not built here any more. They are written into
     events.html by scripts/build-events.js and ship in the HTML, which
     is the only way anything that doesn't run JavaScript — a crawler,
     an answer engine — ever gets to read them. Drawing them here meant
     the page's entire content arrived after a fetch to /api/luma-events,
     and robots.txt disallows /api/.

     What is left is the half that does have to happen in a browser.
     Registration is not ours: each Register button carries Luma's own
     `data-luma-action="checkout"` hook and Luma's script opens their
     modal over the page. We draw the list, they take the RSVP. */

  (function eventsPage() {
    /* No buttons, no script — this keeps it off the other four pages,
       and off /events on a day when nothing is coming up. */
    if (!document.querySelector('[data-luma-action="checkout"]')) return;
